...
- API Test Generator: OpenAPI spec → Gemini generates Rest Assured Java scenarios →
  in-memory `javac` compilation → sandboxed reflective execution → result capture.
- Test Execution Engine: Playwright-for-Java runner with the same compile-and-run pattern,
  async orchestration, and a WebSocket channel (`/ws/test-runs/{id}`) for live status.
...
- UI Locator Generator: `LlmProvider.generateLocators()` exists and has a prompt template,
  but nothing yet calls it to auto-author `UiTestScript.generatedCode` from a TestCase's
...
Without Docker: `cd backend && mvn spring-boot:run` (needs a JDK 17+, not just a JRE —
`javax.tools.ToolProvider.getSystemJavaCompiler()` returns null on a JRE, which breaks
both the API test and UI test execution paths) and `cd frontend && npm install && npm run dev`.

## Known scaffold-level gaps (be aware before treating this as production-ready)

1. **Sandboxing is minimal.** `ApiTestExecutionService` / `PlaywrightExecutionService`
   compile and run LLM-generated code in the same JVM as the API, with only a wall-clock
   timeout. The design doc calls for moving this into the isolated `executor` container
   with no outbound network access beyond the target under test — that container split
   isn't implemented yet, both services just run in the `api` container for now.
