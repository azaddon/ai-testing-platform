...
public class GeminiProvider implements LlmProvider {
    ...
    // An isolated threadpool to keep log repository writes completely out of the HTTP thread path
    //private final ExecutorService loggingExecutor = Executors.newVirtualThreadPerTaskExecutor(); 
    ...
    public GeminiProvider(RestClient.Builder geminiRestClientBuilder,
                           PromptTemplateLoader prompts,
                           LlmCallLogRepository callLogRepository,
                           @Value("${llm.gemini.base-url:https://googleapis.com}") String baseUrl,
        ...
        this.geminiRestClient = geminiRestClientBuilder.baseUrl(baseUrl)
    ...
    public GeneratedTestCases generateTestCases(TestCaseGenRequest request) {
        // 1. Guard rails against malformed or empty incoming data payloads
        if (request == null || request.requirementText() == null || request.requirementText().isBlank()) {
            throw new IllegalArgumentException("Requirement text parameter cannot be null or empty");
        }
        //Safe prompt composition handles non-string primitives cleanly via type generic binding
        ...
        //Execution via safe text delivery routing core configuration
        String raw = callTextModel(fastModel, prompt, 0.3, "test-case-gen");
        String sanitizedJson = cleanMarkdownFences(raw);
        try {
            ...
            throw new LlmResponseParseException("Failed to map JSON payload to GeneratedTestCase DTO properties", sanitizedJson, e);
        }
    }
    ...
    // Removes opening triple backticks and optional language identifier
    String cleaned = input.replaceAll("^```[a-zA-Z]*\\s*", "");
    // Removes closing triple backticks
    ...
    public GeneratedApiTests generateApiTests(ApiTestGenRequest request) {
        if (request == null || request.openApiSpec() == null || request.openApiSpec().isBlank()) {
            throw new IllegalArgumentException("OpenAPI specifications contract cannot be null or empty");
        }
        ...
        try {
            ...
            throw new LlmResponseParseException("Failed to map JSON payload to GeneratedApiTest DTO properties", sanitizedJson, e);
        }
    }

    @Override
    public List<LocatorSuggestion> generateLocators(LocatorGenRequest request) {
        if (request == null || request.domSnapshot() == null || request.targetDescriptions() == null) {
            throw new IllegalArgumentException("DOM snapshot details and target locator description requirements cannot be null");
        }
        ...
        try {
            ...
            throw new LlmResponseParseException("Failed to parse locator suggestions", sanitizedJson, e);
        }
    }
    ...
    public FailureAnalysisResult analyzeFailure(FailureContext context) {
        if (context == null) {
            throw new IllegalArgumentException("Failure context metadata block cannot be null");
        }
        ...
        String sanitizedJson = cleanMarkdownFences(raw);
        try {
            return objectMapper.readValue(sanitizedJson, FailureAnalysisResult.class);
        } catch (Exception e) {
            throw new LlmResponseParseException("Failed to parse failure analysis", sanitizedJson, e);
        }
    }

    @Override
    public String summarizeLogChunk(String logChunk) {
        if (logChunk == null || logChunk.isBlank()) {
            return "No log content provided for summarization.";
        }
        
        String prompt = prompts.render("log-chunk-summary.txt", Map.of("logChunk", logChunk));
        // Simple log summary tasks utilize the cheap, high-speed fastModel tier
    ...
    public ScreenshotAnalysisResult analyzeScreenshot(ScreenshotContext context) {
        if (context == null || context.baselineImageBase64() == null || context.actualImageBase64() == null) {
            throw new IllegalArgumentException("Screenshot context details and source image assets cannot be null");
        }
        ...
        try {
            ...
            String text = extractText(raw);
             String sanitizedJson = cleanMarkdownFences(text);
            ScreenshotAnalysisResult result = objectMapper.readValue(sanitizedJson, ScreenshotAnalysisResult.class);
            logCall(visionModel, "screenshot-analysis", start, true, null);
            return result;
        } catch (RestClientResponseException e) {
            String errorPayload = e.getResponseBodyAsString();
            logCall(visionModel, "screenshot-analysis", start, false, errorPayload);
            ...
            throw new LlmCallException("Systemic error processing imae visual context verification pipeline", e);
        }
    }
    ...
    private String callTextModel(String model, String prompt, double temperature, String feature) {
        ...
        try {
            ...
            //the synchronous HTTP request using RestClient
            String raw = geminiRestClient.post()
                    .uri("/models/{model}:generateContent?key={key}", model, apiKey)
                    .body(requestBody)
                    .retrieve()
                    .body(String.class);
            //get the target generated string
            String text = extractText(raw);
            logCall(model, feature, start, true, null);
            return text;
        } catch (RestClientResponseException e) {
             // Captures direct remote 4xx or 5xx cloud gateway errors
            String errorPayload = e.getResponseBodyAsString();
            logCall(model, feature, start, false, errorPayload);
            throw new LlmCallException("Gemini downstream network call failed for usecase: " + feature, e);
        } catch (Exception e) {
            logCall(model, feature, start, false, e.getMessage());
            throw new LlmCallException("Gemini systematic execution failed for usecase: " + feature, e);
        }
    }
    ...
    private String extractText(String rawResponseJson) throws Exception {
        try {
            ...
            throw new IllegalStateException("The structure returned by the Gemini API did not match expected fields.");
        } catch (Exception e) {
            throw new LlmResponseParseException("Failed to drill through the Gemini envelope JSON structure", rawResponseJson, e);
        }
    }
